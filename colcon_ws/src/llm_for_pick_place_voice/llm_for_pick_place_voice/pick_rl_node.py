...
class PPOPositionControlNode(Node):
    def __init__(self):
        ...
        self.policy_path = os.path.join(package_path, "resource", "spoon_serve_policy.pt")
    ...
    def move_to_point_rl_callback(self, request, response):
        ...
        try:
            ...
            if self.target_cmd is None:
                self.target_cmd = np.array([x, y, 0.35, 1, 0, 0, 0], dtype=np.float32)
            else:
                self.target_cmd[:3] = [x, y, 0.35]
