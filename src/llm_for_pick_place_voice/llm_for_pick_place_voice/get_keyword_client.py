...
class GetKeywordClient(Node):
    ...
    def call_get_target_point(self):
        ...
        get_target_point_future = self.get_target_point_client.call_async(GetTargetPoint.Request())
        rclpy.spin_until_future_complete(self, get_target_point_future)

        try:
            res = get_target_point_future.result()
            self.get_logger().info(
                f"get_target_point service response: "
                f"point=({res.target_point.x}, {res.target_point.y}, {res.target_point.z}), "
                f"class={res.class_name}, confidence={res.confidence}"
            )
            
            # Point를 MoveToPoint 서비스로 전달
            self.forward_point_to_move_to_point(res.target_point)
    ...
    def forward_point_to_move_to_point(self, point):
        ...
        self.get_logger().info(f"Forwarding point to move_to_point service: ({point.x}, {point.y}, {point.z})")
        
        try:
            # MoveToPoint 요청 생성
            req = MoveToPoint.Request()
            req.target_position = point
            ...
            self.get_logger().info(f"Move to point response - Success: {response.success}, Message: {response.message}")
        except Exception as e:
    ...
    def forward_to_other_node(self, message):
        ...
        try:
            ...
            # 모드로 forward_point_to_move_to_point 호출
