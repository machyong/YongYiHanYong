...
class PickPlaceService(Node):
    ...
    def callback(self, request, response):
        ...
        if "cutlery" in cls:
            ...
            z_offset_mm = z * 1000 + 145
            # y_offset_mm = y * 1000 - 25
            pos_z_offset = [x * 1000 -30, y * 1000, z_offset_mm, 90, 180, 0]
            ...
            time.sleep(0.5)
            pos_300 = [x * 1000, y * 1000 -25, 300, 90, 180, 0]
            ...
            pos_z_offset = [x * 1000, y_offset_mm, z_offset_mm, 90, 180, 0]
            self.get_logger().info(f"➡️  물체 위치로 이동 (x, y, z={z_offset_mm}): {pos_z_offset} (z + offset)")
            ret = self.movel(pos_z_offset, VELOCITY, ACC)
            self.get_logger().info(f"movel result: {ret}")
            time.sleep(1.0)
